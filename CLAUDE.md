...
- **2026-06-21** — Hero reverted to a single portrait using `Refeence/Hero.jpeg` (uniform photo) → cropped to `assets/img/hero.jpeg`. The "מי היה מלכיה" gallery now uses the two candid photos instead of the campaign graphics: `assets/img/gallery-smile.jpeg` + `gallery-desert.jpeg` (both cropped 4:5 for a uniform grid). Removed unused `badge.jpeg`, `poster.jpeg`, `hero-smile.jpeg`, `hero-desert.jpeg`. Updated `og:image` to `hero.jpeg`. Verified desktop + mobile. (`photo-smile.jpeg`/`photo-desert.jpeg` kept as full-size sources.)
